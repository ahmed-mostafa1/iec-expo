...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-lg font-semibold">{{ __('Organizer details') }}</h1>
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,0.7fr)]">
        <div class="rounded-xl bg-white border border-gray-200 p-4 text-s space-y-4">
            <div>
                <h2 class="text-sm font-semibold mb-2">{{ __('Overview') }}</h2>
                <dl class="space-y-3">
                    <div class="flex justify-between">
                        <dt class="text-gray-500">{{ __('Name (English)') }}</dt>
                        <dd class="text-gray-900">{{ $organizer->name }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500">{{ __('Name (Arabic)') }}</dt>
                        <dd class="text-gray-900">{{ $organizer->name_ar ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500">{{ __('Website') }}</dt>
                        <dd class="text-gray-900">
                            @if($organizer->url)
                                <a href="{{ $organizer->url }}" class="text-emerald-700 hover:text-emerald-900" target="_blank" rel="noopener">
                                    {{ $organizer->url }}
                                </a>
                            @else
                                {{ __('Not set') }}
                    ...
                    <div class="flex justify-between">
                        ...
                        <dd class="text-gray-900">{{ $organizer->display_order }}</dd>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        ...
                        <dd class="text-gray-900">{{ $organizer->created_at->format('Y-m-d H:i') }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-gray-500">{{ __('Updated at') }}</dt>
                        <dd class="text-gray-900">{{ $organizer->updated_at->format('Y-m-d H:i') }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div>
                ...
                <p class="text-gray-700 whitespace-pre-line">
                    {{ $organizer->description_en ?: __('No English description provided.') }}
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-gray-700 whitespace-pre-line">
                    {{ $organizer->description_ar ?: __('No Arabic description provided.') }}
                </p>
            </div>
        </div>
        ...
        <div class="rounded-xl bg-white border border-gray-200 p-4 text-s">
            ...
            @if($organizer->url)
                <div class="mt-4">
                    <a href="{{ $organizer->url }}" target="_blank" rel="noopener"
