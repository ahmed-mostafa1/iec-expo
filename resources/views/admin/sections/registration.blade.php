            ...
            <section class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-6">
                <div class="grid gap-6 md:grid-cols-2">
                    <div class="space-y-4" data-node-group>
                        <h2 class="text-lg font-semibold text-gray-900">{{ __('Visitor pathway') }}</h2>
                        <div class="rounded-xl border border-gray-200 p-4 bg-gradient-to-br from-gray-50 to-white space-y-3">
                            <button type="button" class="w-full text-left" data-node-trigger="visitor-card">
                                <p class="text-xs uppercase font-semibold text-emerald-700">{{ __('Card block') }}</p>
                            ...
                            <div class="space-y-4 hidden" data-node-panel="visitor-card">
                                ...
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('CTA Label (EN)') }}</label>
                                    <input type="text" name="visitor_card[cta_label][en]"
                                        value="{{ old('visitor_card.cta_label.en', data_get($visitorCard, 'cta_label.en')) }}"
                                        class="w-full mt-1 rounded-lg border-gray-200 text-sm" />
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('CTA Label (AR)') }}</label>
                        ...
                        <div class="rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/30 p-4 space-y-4" data-node-group>
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ __('Click any field stub to edit labels, placeholders, and hints.') }}</p>
                                </div>
                                <button type="button" class="text-xs font-semibold text-emerald-700" data-node-trigger="visitor-form-meta">
                                    {{ __('Edit header') }}
                                </button>
                            </div>
                            ...
                            <div class="rounded-xl border border-gray-200 bg-white p-4 space-y-4 hidden" data-node-panel="visitor-form-meta">
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Contact link (EN)') }}</label>
                                    ...
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Contact link (AR)') }}</label>
                                ...
                                @foreach ($visitorFields as $index => $field)
                                    <div class="rounded-xl border border-gray-200 bg-white p-3">
                                        <button type="button" class="flex w-full items-center justify-between text-left text-sm" data-node-trigger="visitor-field-{{ $index }}">
                                            <div>
                                                <p class="text-xs font-semibold text-gray-500 uppercase">{{ __('Field') }} #{{ $loop->iteration }}</p>
                                                <p class="font-semibold text-gray-900">{{ data_get($field, 'label.en') }}</p>
                                                <p class="text-xs text-gray-500">{{ strtoupper($field['type'] ?? 'text') }}</p>
                                            </div>
                                        ...
                                        <div class="mt-3 space-y-3 hidden" data-node-panel="visitor-field-{{ $index }}">
                                            ...
                                            <div>
                                                <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Label (EN)') }}</label>
                                            ...
                                            <div>
                                                <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Label (AR)') }}</label>
                                            ...
                                            <div>
                                                <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Placeholder (EN)') }}</label>
                                            ...
                                            <div>
                                                <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Placeholder (AR)') }}</label>
                                            ...
                                            @if (($field['hint']['en'] ?? null) !== null || ($field['hint']['ar'] ?? null) !== null || ($field['type'] ?? null) === 'file')
                                                <div>
                                                    <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Hint (EN)') }}</label>
                                                    <input type="text" name="visitor_form[fields][{{ $index }}][hint][en]"
                                                        value="{{ old("visitor_form.fields.$index.hint.en", data_get($field, 'hint.en')) }}"
                                                        class="w-full mt-1 rounded-lg border-gray-200 text-sm" />
                                                </div>
                                                <div>
                                                    <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Hint (AR)') }}</label>
                                            ...
                                            @if (($field['type'] ?? '') === 'select')
                                                <div class="space-y-3">
                                                    <p class="text-xs font-semibold text-gray-500 uppercase">{{ __('Options') }}</p>
                        ...
                        <div class="rounded-2xl border border-dashed border-indigo-200 bg-indigo-50/30 p-4 space-y-4" data-node-group>
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ __('Capture both steps using the skeleton layout.') }}</p>
                                </div>
                                <button type="button" class="text-xs font-semibold text-indigo-700" data-node-trigger="exhibitor-form-meta">
                                    {{ __('Edit header') }}
                                </button>
                            </div>
                            ...
                            <div class="rounded-xl border border-gray-200 bg-white p-4 space-y-4 hidden" data-node-panel="exhibitor-form-meta">
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Step one label (EN)') }}</label>
                                        <input type="text" name="exhibitor_form[step_one][en]"
                                            value="{{ old('exhibitor_form.step_one.en', data_get($exhibitorForm, 'step_one.en')) }}"
                                            class="w-full mt-1 rounded-lg border-gray-200 text-sm" />
                                    </div>
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Step one label (AR)') }}</label>
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Step two label (EN)') }}</label>
                                        <input type="text" name="exhibitor_form[step_two][en]"
                                            value="{{ old('exhibitor_form.step_two.en', data_get($exhibitorForm, 'step_two.en')) }}"
                                            class="w-full mt-1 rounded-lg border-gray-200 text-sm" />
                                    </div>
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Step two label (AR)') }}</label>
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Next CTA (EN)') }}</label>
                                        <input type="text" name="exhibitor_form[cta_next][en]"
                                            value="{{ old('exhibitor_form.cta_next.en', data_get($exhibitorForm, 'cta_next.en')) }}"
                                            class="w-full mt-1 rounded-lg border-gray-200 text-sm" />
                                    </div>
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Next CTA (AR)') }}</label>
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Submit CTA (EN)') }}</label>
                                    ...
                                    <div>
                                        <label class="text-xs font-semibold text-gray-500 uppercase">{{ __('Submit CTA (AR)') }}</label>
                            ...
                            <div class="space-y-4" data-node-group>
                                <p class="text-xs font-semibold text-gray-500 uppercase">{{ __('Step one fields') }}</p>
                            ...
                            <div class="space-y-4" data-node-group>
                                <p class="text-xs font-semibold text-gray-500 uppercase">{{ __('Step two fields') }}</p>
