...
@section('content')
    <h1 class="text-lg font-semibold mb-4">
        {{ __('Icon Plus registration') }} #{{ $registration->id }}
    </h1>
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.2fr)]">
        ...
        <div class="space-y-4">
            <div class="rounded-xl bg-white border border-gray-200 p-4 text-s">
                ...
                <form method="POST" action="{{ route('admin.icon-plus.update-status', $registration) }}" class="space-y-2">
                    @csrf
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        @if($registration->pdf_path)
                            <a href="{{ route('admin.icon-plus.download-pdf', $registration) }}"
                               class="text-[11px] text-emerald-700 hover:text-emerald-900">
                                {{ __('Download PDF') }}
                            </a>
                        @else
                    ...
                    <form method="POST" action="{{ route('admin.icon-plus.regenerate-pdf', $registration) }}">
                        @csrf
                        <button
                            type="submit"
